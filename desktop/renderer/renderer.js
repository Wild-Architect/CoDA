...
function requestEdessbRevision(typeId, revisionNumber) {
  return new Promise(resolve=>{
    const dialog=$('#edessb-document-dialog'),type=$('#edessb-document-type'),url=$('#edessb-document-url'),error=$('#edessb-document-error'),pdfName=$('#edessb-document-pdf-name'),clearPdf=$('#edessb-document-clear-pdf');let done=false,sourcePath='';
    const finish=value=>{if(done)return;done=true;if(dialog.open)dialog.close();resolve(value);};
    type.innerHTML=edessbConfig.documentTypes.map(item=>`<option value="${escape(item.id)}">${escape(item.label)} (${escape(item.short)})</option>`).join('');type.value=typeId;url.value='';error.hidden=true;pdfName.textContent='Файл не обрано';clearPdf.hidden=true;$('#edessb-document-dialog-title').textContent=`Редакція №${revisionNumber}`;
    $('#edessb-document-select-pdf').onclick=async()=>{try{const selected=await window.ekp.selectEdessbPdf();if(selected.canceled)return;sourcePath=selected.sourcePath;pdfName.textContent=selected.name;clearPdf.hidden=false;error.hidden=true;}catch(reason){error.textContent=readableError(reason);error.hidden=false;}};
    clearPdf.onclick=()=>{sourcePath='';pdfName.textContent='Файл не обрано';clearPdf.hidden=true;};
    $('#edessb-document-cancel').onclick=()=>finish(null);
    $('#edessb-document-accept').onclick=()=>{const payload={type:type.value,url:url.value.trim(),sourcePath};if(!payload.url&&!payload.sourcePath){error.textContent='Додайте посилання, PDF-файл або обидва.';error.hidden=false;return;}if(payload.url){let parsed;try{parsed=new URL(payload.url);}catch{error.textContent='Вкажіть правильне посилання на документ.';error.hidden=false;url.focus();return;}if(parsed.protocol!=='https:'||!(parsed.hostname==='e-construction.gov.ua'||parsed.hostname.endsWith('.e-construction.gov.ua'))){error.textContent='Посилання повинно вести на захищену сторінку e-construction.gov.ua.';error.hidden=false;url.focus();return;}}finish(payload);};
